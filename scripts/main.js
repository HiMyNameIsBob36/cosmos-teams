...
world.beforeEvents.chatSend.subscribe((data) => {
    ...
    system.run(() => {
        ...
        switch (cmd) {
            case "create":
                if (myTeamName) return sender.sendMessage("§l§c- §r§fYou are already in a team!");
                const name = args[2];
                if (!name) return sender.sendMessage("§l§c- §r§fUsage: .team create {name}");
                teams[name] = { 
                    owner: sender.name, 
            ...
            case "info":
                if (!team) return sender.sendMessage("§l§c- §r§fYou have no team.");
                const infoUser = findMemberInTeam(team, args[2]);
                const infoP = world.getAllPlayers().find(p => p.name === infoUser);
                if (!infoP) return sender.sendMessage("§l§c- §r§fTeam member is offline.");
            ...
            case "request":
                ...
                if (!targetT) return sender.sendMessage("§l§c- §r§f Team not found.");
            ...
            case "invites":
                if (!team || !team.managers.includes(sender.name)) return sender.sendMessage("§l§c- §r§f Manager only.");
            ...
            case "accept":
            case "decline":
                if (!team || !team.managers.includes(sender.name)) return sender.sendMessage("§l§c- §r§f You must be a manager");
                const idInput = parseInt(args[2]);
                const reqObj = team.requests.find(r => r.id === idInput);
                if (!reqObj) return sender.sendMessage("§l§c- §r§f Invalid Request ID.");
            ...
            case "tp":
                if (!team) return sender.sendMessage("§l§c- §r§f You have no team.");
                if (args[2] === "accept") {
                    const reqName = tpRequests.get(sender.name);
                    if (!reqName) return sender.sendMessage("§l§c- §r§f No pending TP requests.");
                ...
                } else sender.sendMessage("§l§c- §r§f Player not online.");
                break;

            case "home":
                if (!team) return sender.sendMessage("§l§c- §r§f You have no team.");
                if (args[2] === "set") {
                    if (team.owner !== sender.name) return sender.sendMessage("§l§c- §r§f Owner only.");
                    team.home = { x: sender.location.x, y: sender.location.y, z: sender.location.z };
                    saveTeams(teams);
                    sender.sendMessage("§l§a+ §r§f Home saved!");
                } else {
                    if (!team.home) return sender.sendMessage("§l§c- §r§f Home not set.");
            ...
            case "kick":
                ...
                if (!kickTarget) return sender.sendMessage("§l§c- §r§f Member not found.");
                if (kickTarget === team.owner) return sender.sendMessage("§l§c- §r§f You can't kick the owner!");
                
                team.members = team.members.filter(m => m !== kickTarget);
                team.managers = team.managers.filter(m => m !== kickTarget);
                saveTeams(teams);
                sender.sendMessage(`§l§c- §r§f Kicked ${kickTarget}.`);
                break;

            case "manager":
                if (!team || team.owner !== sender.name) return sender.sendMessage("§l§c- §r§f You must be the owner.");
            ...
            case "chat":
                if (!myTeamName) return sender.sendMessage("§l§c- §r§f You have no team.");
                if (sender.hasTag("teamChat")) {
                    sender.removeTag("teamChat");
                    sender.sendMessage("§l§c- §r§f Team chat §l§cdeactivated.");
                } else {
            ...
            case "leave":
                if (!team) return;
                if (team.owner === sender.name) return sender.sendMessage("§l§c- §r§f Use '.team disband' as owner.");
                team.members = team.members.filter(m => m !== sender.name);
                team.managers = team.managers.filter(m => m !== sender.name);
                saveTeams(teams);
                sender.sendMessage("§l§c- §r§f You left the team.");
                break;

            case "disband":
                if (!team || team.owner !== sender.name) return sender.sendMessage("§l§c- §r§f Owner only.");
                if (args[2] !== "confirm") return sender.sendMessage("§6§l! §r§f Type '.team disband confirm' to delete.");
                delete teams[myTeamName];
                saveTeams(teams);
                sender.sendMessage("§l§a+ §r§f Team disbanded.");
                break;
